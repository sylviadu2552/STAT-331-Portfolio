...
-   numeric
    -   Although not directly related to filtering the numeric data type, I utilized across in the mutate function to make the the code less repetitive and easier for readers to comprehend.
...
-   date (example must use functions from **lubridate**)
    ...
    -   Used %within% to check if a date is the interval of concert dates.

```{r}
#| label: wd-3-date

# Lab 5 - Finding the New Suspect (Revised)
concert_attendees <- facebook_event_checkin |>
  mutate(date = ymd(date), 
         concert_interval = interval(ymd("2017-12-01"), ymd("2017-12-31"))) |>
  filter(event_name == "SQL Symphony Concert",
         date %within% concert_interval) |>
  group_by(person_id) |>
...
-   character -- specifically a string (example must use functions from **stringr**)
    -   Revised to utiliize the modification of a character type variable. I added an initials variable that gives the initials of a name. For example, the name "Annabel Miller" would return "AM".

```{r}
#| label: wd-4-string

# Lab 5 - Narrowing Down The Witnesses (Revised)
witnesses <- person |>
  filter((address_street_name == "Northwestern Dr") | 
         (address_street_name == "Franklin Ave" & str_detect(name, "Annabel"))) |>
  mutate(
    initials = str_extract_all(name, "\\b[A-Z]") |> 
...
-   date (example must use functions from **lubridate**)
    ...
    -   Further revised to utilize an interval instead of checking by month and date separately. This makes it more readable as it is more clear that I am looking for dates within December 2017.
...
-   `right_join()`
    -   Revised in Lab 4 to better utilize right_join.
        -   By using a right join, it ensures that all possible combinations of the study years (2008 and 2018) from the childcare_costs dataset remains, even if some are missing in the dataset ca_counties. This would be different from an inner join as an inner join would only include combinations that are included in both childcare_costs and ca_counties.
            -   I wouldn't use an inner join here since some rows might be excluded, which would leave potential gaps in my analysis.

```{r}
#| label: wd-5-right

# Lab 4 - Question 4 (Revised)
median_income_by_region <- childcare_costs |>
  filter(state_name == "California" & study_year %in% c(2008, 2018)) |>
  right_join(ca_counties, by = c("county_fips_code", "region")) |>
...
I believe I've done this consistently throughout the assignments and lab in class as I ensure all file paths are related to the project directory, which is organized into sub-folders for easier reproducibility. I also ensure the rendered document looks good by utilizing YAML options as needed. An example of this that I utilized often was "echo: true" to make sure the code shows in the rendered document and "embed-resources: true" to make sure visuals like tables and plots are added directly to the rendered document. One specific lab that I'm extremely proud of in terms of reproducibility is:

-   Lab 5

    -   I believe that with the additional touch of headings and line breaks, the formatting was much cleaner, allowing readers to follow along the steps of solving the crime more easily.
...
-   Example of function formatting
    -   Renamed "range_x" to "range_vec" to make it more tidy and clear that the range referencing to the vector input. Same process was done for min and max.
...
-   at least one numeric variable and one categorical variable
    -   Used the box plots from Lab 2 that was vertically stacked instead of horizontally stacked so readers don't have to tilt their head.
    -   Removed the y - axis label since the title already explains the y-axis is and readers won't have to tilt their heads.
...
long_childcare <- ca_counties |>
  ...
  pivot_longer(cols = c(Infants, Toddlers, Preschoolers),
               names_to = "age_group",
               values_to = "median_price") |>
  mutate(region = fct_reorder2(region, study_year, median_price))

colors <- colorRampPalette(brewer.pal(8, "RdBu"))(10)
...
-   I can reorder my legend to align with the colors in my plot
    -   Reordered through refactoring the levels of region with a function (and not hard coding the ordering)
...
colors <- colorRampPalette(brewer.pal(8, "Set3"))(10)
...
-   I can use annotations
    -   Revised to make use of annotations
        -   Re-revised to remove the legend. The legend is useless and just takes up space when the annotations already provides the information needed to readers.
...
-   Example using `across()`
    -   Revised for more efficient and tidy coding. Removed the use of mutate() since summarise() is already calculating the missing values. Also changed the code so it didn't save to a variable as there was no need for me to save it as a result.
...
BlackfootFish |>
  summarise(total_missing_observations = sum(if_any(everything(), is.na)), 
            across(everything(), ~ sum(is.na(.)), .names = "missing_in_{.col}"))
...
-   Example 1
    -   Revised to add colors to my original challenge 9 table
...
prop_table <- prop |>
  ...
  pivot_wider(names_from = ncorrect, 
              ...
              names_prefix = "Number Correct: ")

# Challenge 9
prop_table |> 
  gt() |>
  tab_header(
    title = "Proportion of Simulations with Correct Number of Babies",
    subtitle = "Summary of proportions for 0, 1, 2, 3, and 4 correct returns"
  ) |> 
  tab_style(
    style = list(
      cell_text(font = "Times New Roman", size = 14, color = "black")
    ),
    locations = cells_body()
  ) |>
  tab_style(
    style = list(
      cell_borders(side = "all", color = "black", weight = 1)
    ),
    locations = cells_body()
  ) |> 
  tab_style(
  ...
  tab_style(
    style = list(
      cell_fill(color = "lightblue3") 
    ),
    locations = cells_title()
  ) |>
  tab_style(
    style = list(
      cell_borders(side = "bottom", color = "black", weight = 2)
    ),
    locations = cells_column_labels()
  ) |> 
  fmt_percent(
    columns = starts_with("Number Correct: "),
    decimals = 2
  )
```

-   Example 2
    -   Revised to add a table header, change the overall font size, and style the column labels.
...
missing_table <- data.frame(Missing_Values = missing_values)            

#Challenge 9 (Revised)
missing_table |> 
  kable(col.names = c("Variable Name", "Missing Values"), 
        format = "html") |> 
  row_spec(0, bold = TRUE, color = "black", background = "grey80") |>  
  kable_styling(font_size = 10) |>
  add_header_above(c("Missing Data Information" = 2))
```
...
-   I can use functions which are not superseded or deprecated
    -   Using if_any() replaces the use of filter(across()), which is superseded.
...
-   I can connect a data wrangling pipeline into a `ggplot()`
    -   Connected my pipeline code block with my ggplot code block using a pipe. This allows for my desired result to output in one code block and only refer to the dataframe once instead of referring to it both in the pipeline and the ggplot.
...
randomBabies <- function(nBabies){
  ...
  babies <- sample(1:nBabies, nBabies, replace = FALSE)
  sum(babies == 1:nBabies)
}
...
chisq_result <- chisq.test(teacher_evals_compare$SET_level, 
...
Throughout the course, I did the challenges, and also took what I learned in this class to my Stat 305 class! For a project, we had to make a graph, and if we knew ggplot, we were given the option to use it if we want. I also learned how to better utilize the resources provided to us, which helped a lot when I was creating tables for challenge 9. <br><br>

For the portfolio parts that we didn't learn yet for the midterm, although we didn't learn across yet, I used resources like ChatGPT and R Documentation to learn how the across function works. I also made revisions and provided hypothetical examples to utilize some of the functions we haven't used yet, like anti_join or full_join.
